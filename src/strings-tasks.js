...
function removeFirstOccurrences(str, value) {
  ...
  return substrIndex !== -1
    ? str.slice(0, substrIndex) + str.slice(substrIndex + value.length)
    : str;
}
...
function sumOfCodes(str) {
  return typeof str === 'string'
    ? str.split('').reduce((sum, element) => sum + element.charCodeAt(0), 0)
    : '';
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((acc, letter) => (vowels.includes(letter) ? acc + 1 : acc + 0), 0);
}
...
function isPalindrome(str) {
  const regExp = /[!?,.]/g;
  const formatStr = str.toLowerCase().replaceAll(' ', '').replace(regExp, '');
  const reverseStr = formatStr.split('').reverse().join('');
  return reverseStr === formatStr;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((letter) => {
      if (alphabetSmall.includes(letter)) {
        return alphabetSmall.at(alphabetSmall.indexOf(letter) - 13);
      }
      if (alphabetBig.includes(letter)) {
        return alphabetBig.at(alphabetBig.indexOf(letter) - 13);
      }
      return letter;
    })
    .join('');
